...
- **`libs/observability-starter`:** auto-configuration providing `/healthz`, `/readyz` (every registered Spring Boot `HealthIndicator` except liveness — plan deviation 1), `/actuator/prometheus`, structured JSON logging with `traceId`/`spanId`/`userId`, W3C trace propagation, and a `Clock` bean. Unit-tested with a slice test. — **Deferred:** userId joins the MDC with the first authenticated request filter (spec 004).
...
- **Compose:** `compose.yaml` with `postgres`, `redis`, `kafka` (KRaft) + `schema-registry`, `keycloak` (realm import), `mailpit`, `prometheus`, `grafana` (provisioned datasources + an empty "Sale Overview" dashboard), `tempo`, `loki`, `inventory-service`, `psp-simulator`; health checks + `depends_on: service_healthy`; `infra` and `full` profiles; named volumes; resource limits. `.env.example` documents every variable. — **As built:** backing services run without a profile (the implicit infra set) and `--profile full` adds the services; memory limits apply to the two application services, backing-service limits follow in 005.
...
## Tests
- Unit: `@ConfigurationProperties` validation (missing/invalid → error naming the property); `/healthz` and `/readyz` handlers with a toggleable stub `HealthIndicator`; `Clock` injection. JSON log layout includes trace fields — **Deferred:** covered when 004 adds trace/log correlation.
- Architecture: ArchUnit rules for the hexagonal layout and no module cycles (a deliberately violating test module proves the rule fires, then is deleted in the same commit) — **As built:** proven red during Task 6 with a temporary violating class, then deleted; package cycles are checked by `noPackageCycles`.
...
- Security: 401 matrix on `GET /events/{id}`; tampered-signature and wrong-audience tokens rejected — **Deferred:** a `RealDecoderValidationTest` exercising the real decoder was attempted for the final-review fix wave but is blocked (Boot 4.1.1's issuer-uri-based decoder supersedes the public-key-location decoder once `sellout.oidc.issuer` is set, which the bridge always does; see `final-fix-report.md`). Wrong-audience and wrong-issuer rejection remain characterised only via the mocked `JwtDecoder` in `ResourceServerTest`.
